...
export function getFilteredMessages(messages, query) {
  ...
  return messages.filter((message) => {
    const attachmentName = message.attachment?.name?.toLowerCase() || '';
    return (message.text || '').toLowerCase().includes(value) || attachmentName.includes(value);
  });
}
...
export default function useConversations(user, navigate) {
  ...
  const sendMessage = React.useCallback(async (conversationId, text, attachment) => {
    const trimmedText = text.trim();
    if (!conversationId || (!trimmedText && !attachment) || sendingMessage) return false;
    setSendingMessage(true);
    try {
      const message = await convApi.sendMessage(conversationId, { text: trimmedText, attachment });
