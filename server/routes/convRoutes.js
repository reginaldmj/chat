...
router.get('/', (req, res) => {
  ...
  const result = convs.map((conv) => {
    const msgs = db.getMessages(conv.id, 1);
    const last = msgs[msgs.length - 1] || null;
    const unread = db.unreadCount(conv.id, req.user.id);
    ...
    return {
      ...
      members,
      lastMessage: last
        ? { text: last.text || last.attachment?.name || 'Attachment', time: formatTime(last.createdAt), senderId: last.senderId }
        : null,
